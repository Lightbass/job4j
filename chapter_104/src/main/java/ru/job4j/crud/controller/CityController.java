...
public class CityController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        Long countryId = -1L;
        try {
            countryId = Long.parseLong(req.getParameter("countryId"));
        } catch (NumberFormatException nfe) {
            LOGGER.error(nfe.getMessage(), nfe);
        }
        encodeJson(resp.getOutputStream(), logic.findCitiesByCountryId(countryId));
    }

    private void encodeJson(OutputStream out, Collection<City> countries) {
        try {
